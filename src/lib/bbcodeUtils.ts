import { applyUnicodeFont, type UnicodeFontKey } from "./unicodeFonts";
...
export const generatePreviewHTML = (bbcode: string) => {
    return bbcode
        ...
        .replace(
            /\[size=(\d+)\]/g,
            (_, size: string) =>
                `<span style="font-size:${PX_MAP[size] || "16px"}">`,
        )
...
export interface GenerateBBCodeOptions {
    text: string;
    effect: BBCodeEffect;
    startColor: string;
    middleColor: string;
...
export const generateBBCode = ({
    ...
    bold,
    italic,
}: GenerateBBCodeOptions) => {
    const processedText = applyUnicodeFont(text, font);
