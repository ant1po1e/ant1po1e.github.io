...
const markdownComponents: Components = {
    ...
    li: ({ ...props }) => <li className="text-ink/80" {...props} />,
    blockquote: ({ ...props }) => (
        <blockquote
    ...
    code: ({ ...props }) => {
        ...
        return isInline ? (
            ...
            <code className="font-mono text-xs text-ink" {...props} />
        );
    },
    pre: ({ ...props }) => (
        <pre
    ...
    td: ({ ...props }) => (
        <td className="border border-rule px-2 py-1" {...props} />
    ),
};
...
export const ArticleSection = () => {
    ...
    useEffect(() => {
        Promise.all(
            ARTICLE_FILES.map((file) => fetch(file).then((res) => res.text())),
        ).then((markdowns) => {
