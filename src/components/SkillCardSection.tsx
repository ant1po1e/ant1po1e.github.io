...
import { SkillRadarChart, type SkillValues, type ThemeColors } from "./SkillRadarChart";
...
export const SkillCardSection = () => {
    ...
    // Read persisted values only after mount, so the server-rendered markup
    // and the first client render match (localStorage doesn't exist on the
    ...
    useEffect(() => {
        try {
            setAvatar(window.localStorage.getItem("mania-avatar") || "");
            setTheme(window.localStorage.getItem("mania-theme") || "default");
        } catch {
    ...
    const handleThemeChange = (value: string) => {
        setTheme(value);
        try {
            window.localStorage.setItem("mania-theme", value);
        } catch {
    ...
    const handleAvatarUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = () => {
            ...
            try {
                window.localStorage.setItem("mania-avatar", result);
            } catch {
                // ignore storage errors
            }
            setAvatar(result);
