...
function fileKind(name: string): "image" | "video" | "audio" | "other" {
...
function TableRow({ image, onSelect, onDelete }: TableRowProps) {
...
export const VaultSection = () => {
    ...
    function handleUploaded(blob: PutBlobResult) {
        setImages((prev) => [
            {
                url: blob.url,
    ...
    async function handleDelete(image: VaultImage) {
        try {
